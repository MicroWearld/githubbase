...
# 背景
game = t.Screen()
game.title('pong v1.1.0')
game.bgcolor('black')
game.setup(800, 600)
# 球拍
p1 = hitbox(7, 7, 0, 0, "white")
# 乒乓
pp = hitbox(1, 1, -100, -100, "orange")
pp.down()
# 信息板
...
def reset():
    ...
    speed.clear()
    cboard.write(f"{p1_count}:{p2_count}",
                 align="center", font=("Terminal", 90))
    main()
...
def main():
    ...
    use1.clear()
    use2.clear()
    speed.write(f"speed:{abs(round(pp_cx,2))}",
                align="center", font=("Terminal", 15))
    time.sleep(0.5)
    while p1_count < 3 and p2_count < 3:
        ...
        if pp.ycor() > 280 or pp.ycor() < -280:
            pp_cy *= -1
        if hitcheck(p1, pp):
            # print("lmax:",(pp.bx+p1.bx)/(pp.by+p1.by))
            # print("l:",abs(pp.xcor()-p1.xcor())/abs(pp.ycor()-p1.ycor()))
            # print("\n")
